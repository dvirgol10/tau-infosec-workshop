...
{ // helper source: https://man7.org/linux/man-pages/man3/localtime.3p.html
	struct tm tmstmp;
...
void print_reason(reason_t reason)
{
	if ((int)reason < 0)
	{
		...
		case REASON_ILLEGAL_VALUE:
			...
			return;
		case REASON_MATCHING_CONN_ENTRY:
			printf("REASON_MATCHING_CONN_ENTRY ");
			return;
		case REASON_NO_MATCHING_CONN_ENTRY:
			printf("REASON_NO_MATCHING_CONN_ENTRY ");
			return;
...
char *state_to_s(state_t state)
{
	...
	case ESTABLISHED:
		return "ESTABLISHED";
	case FIN_1_RECEIVED:
		return "FIN_1_RECEIVED";
	case FIN_2_RECEIVED:
		return "FIN_2_RECEIVED";
	}
	print_error_message_and_exit("There is some state which isn't valid");
	return ""; // we should never reach this
}
...
// prints the metadata of a tcp connection as it stored in the dynamic connection table

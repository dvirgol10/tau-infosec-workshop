...
void print_reason(reason_t reason) {
	if ((int)reason < 0) {
		switch (reason) {
			...
			case REASON_MATCHING_CONN_ENTRY: printf("REASON_MATCHING_CONN_ENTRY "); return;
			case REASON_NO_MATCHING_CONN_ENTRY: printf("REASON_NO_MATCHING_CONN_ENTRY "); return;
			case REASON_ALREADY_HAS_CONN_ENTRY: printf("REASON_ALREADY_HAS_CONN_ENTRY "); return;
		}
...
char* state_to_s(state_t state) {
	switch (state) {
		...
		case FIN_1_RECEIVED: return "FIN_1_RECEIVED";
		case FIN_2_RECEIVED: return "FIN_2_RECEIVED";
	}

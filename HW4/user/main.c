...
void load_rules(char *path_to_rules_file)
{
	...
	if (write(fd, rule_table, num_rules * sizeof(rule_t)) != num_rules * sizeof(rule_t))
	{ // sends the rule table to the firewall module
		close(fd);
		perror("write");
		exit(1);
	}
...
void show_log()
{
	...
	{
		if (bytes_read != sizeof(log_row_t))
		{ // we read a single log row every time
			close(fd);
			perror("read");
			exit(1);
		}
...
int main(int argc, char *argv[])
{
	...
	else if (argc == 3 && !strcmp(argv[1], "load_rules"))
	{ // "load_rules" is the only command which needs another argument
		load_rules(argv[2]);
		return 0;
	}

...
int match_tcphdr(rule_t* rule, struct tcphdr* hdr) {
	return match_port(rule->src_port, hdr->source) && match_port(rule->dst_port, hdr->dest) && match_ack(rule->ack, tcp_flag_word(hdr) & TCP_FLAG_ACK);
}
...
int match_rule(rule_t* rule, struct sk_buff* skb, direction_t pkt_direction) {
	...
	switch (hdr->protocol) {
		...
		case PROT_TCP: return match_tcphdr(rule, tcp_hdr(skb));

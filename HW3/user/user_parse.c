...
int parse_ip_string(char* ip_addr_and_prefix_size, unsigned int* ip, unsigned char* prefix_size, unsigned int* prefix_mask) {
	...
	*ip = ip_convert_string_to_network(token);
	...
	if (!only_digits_string(token)) { // the mask part should contain only digits
		return 0;
	}
	...
	*prefix_size = (unsigned char) maybe_prefix_size;
	...
	if (token = strtok_r(NULL, " ", &ip_save_ptr)) { // if there is more data in the line despite we've completed the ip address string, it's invalid
		return 0;
	}
...
int parse_port_string(char* port_string, unsigned short* port) {
	...
	if (!strcmp(port_string, ">1023")) { // this is the string of "above 1023"
		*port = PORT_ABOVE_1023;
		return 1;
	}
	if (!only_digits_string(port_string)) { // if the port != "any" and != ">1023", it should be a number
		return 0;
	}
...
int parse_rule(char* rule_line, int rule_num) {
	...
	if (strlen(token) > 20) { // names will be no longer than 20 chars
		return 0;
	}
	...
	if (token = strtok_r(NULL, " ", &rule_token_save_ptr)) { // if there is more data in the line despite we've completed the rule, it's invalid
		return 0;
	}
	return 1;
}
...
void parse_rules_file(char* path_to_rules_file) { // helper source for reading lines: https://stackoverflow.com/a/3501681 
	FILE* fp;
	char* line = NULL;
	size_t len = 0;
	int rule_num = 0;
	...
	while (getline(&line, &len, fp) != -1) {
		if (rule_num >= MAX_RULES) { // we allow only MAX_RULES rules
			fclose(fp);
			if (line) {
				free(line);
			}
		...
		if (!parse_rule(line, rule_num)) { // parse each rule separately
